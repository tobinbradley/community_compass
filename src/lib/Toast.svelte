...
>
  ...
  <div>
    {#if $toastMessage.reloadButton}
      <button class="px-4 py-2 text-sm uppercase ml-4 leading-none border text-white border-white hover:border-transparent hover:text-sky-900 hover:bg-white transition-colors duration-200 ease-in-out" on:click={() => location.reload()} on:keydown={() => location.reload()}>reload</button>
    {/if}
  </div>
</div>
